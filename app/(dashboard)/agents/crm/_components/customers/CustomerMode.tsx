    ...
    <>
      ...
      {stats && (
        <div className="grid grid-cols-5 gap-3 mb-6">
          ...
          />
          <KPICard label="Avg Days in Stage" value={String(stats.avgDaysInStage)} />
        </div>
      )}
                    ...
                    <div className="space-y-2">
                      {stageGroups[stage].map((deal) => (
                        <button
                          ...
                          style={{
                            cursor: "pointer",
                            borderColor: selectedDeal?.id === deal.id ? `${ACCENT}60` : undefined,
                          }}
                        >
          ...
          {(streamedOutput || isRunning) && (
            <div className="bg-card-bg border border-border-default rounded-card p-6">
              ...
              >
                {streamedOutput}
                {isRunning && (
                  <span className="inline-block w-2 h-4 ml-1 animate-pulse" style={{ backgroundColor: ACCENT }} />
                )}
              </div>
              {streamedOutput && !isRunning && (
                <div className="mt-3">
                  <DownloadButton content={streamedOutput} filename="crm-analysis" formats={["md", "txt"]} accentColor={ACCENT} />
                </div>
              )}
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card-bg border border-border-default rounded-card p-5">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-text-heading text-sm font-semibold font-display">Data Sync</div>
            ...
            {syncResult && (
              <div
                ...
                style={{
                  ...
                  border: syncResult.error ? "1px solid rgba(239,68,68,0.2)" : "1px solid rgba(16,185,129,0.2)",
            ...
            {showSyncLog && syncLogs.length > 0 && (
              <div className="mt-3">
                {syncLogs.map((log) => (
                  ...
                  >
                    <span className="text-text-dim">{new Date(log.startedAt).toLocaleString()}</span>
                    <span
                      style={{
                        color:
                          log.status === "completed" ? "#6EE7B7" : log.status === "failed" ? "#FCA5A5" : "#FBBF24",
                      }}
                    >
                      {log.status}
                    </span>
            ...
            <div className="p-6 space-y-5">
              ...
              {analyzingDeal && (
                <div className="bg-card-bg border border-border-default rounded-card p-6 text-center">
                  ...
                  />
                  <p className="text-text-dim text-xs font-mono">Analyzing deal intelligence...</p>
                </div>
              )}
                    ...
                    <div className="bg-card-bg border border-border-default rounded-card p-4">
                      ...
                      <div className="space-y-3">
                        {dealAnalysis.recommendations.map((rec, i) => (
                          <div key={i} className="bg-background border border-border-default rounded-lg p-3">
                            <div className="flex items-start justify-between gap-2">
                              <p className="text-xs text-foreground font-medium leading-snug">{rec.action}</p>
                              <UrgencyBadge urgency={rec.urgency} />
                            </div>
                            <p className="text-[11px] text-text-body mt-1.5 leading-relaxed">{rec.reasoning}</p>
