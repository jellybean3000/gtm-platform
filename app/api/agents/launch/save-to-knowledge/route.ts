...
export async function POST(request: NextRequest) {
  ...
  try {
    const [doc] = await db
      ...
      .values({
        teamId,
        filename: title || `Launch Plan - ${new Date().toLocaleDateString()}`,
        fileType: "agent-output",
        status: "processing",
        uploadType: "integration",
        metadata: { source: "launch-agent", runId },
      })
    ...
    const chunkValues = chunks.map((text, index) => ({
      ...
      chunkIndex: index, classification: { type: "launch_planning", source: "agent-output" },
    }));
