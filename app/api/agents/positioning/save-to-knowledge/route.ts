...
export async function POST(request: NextRequest) {
  ...
  try {
    const [doc] = await db
      ...
      .values({
        teamId,
        filename: title || `Positioning - ${new Date().toLocaleDateString()}`,
        fileType: "agent-output",
        status: "processing",
        uploadType: "integration",
        metadata: { source: "positioning-agent", runId },
      })
    ...
    const chunkValues = chunks.map((text, index) => ({
      ...
      chunkIndex: index,
      classification: { type: "messaging_heritage", source: "agent-output" },
    }));
