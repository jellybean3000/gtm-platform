...
export async function POST(request: NextRequest) {
  ...
  try {
    const [doc] = await db
      ...
      .values({
        teamId,
        filename: title || `Sales Enablement - ${new Date().toLocaleDateString()}`,
        fileType: "agent-output",
        status: "processing",
        uploadType: "integration",
        metadata: { source: "sales-enablement-agent", runId },
      })
    ...
    const chunkValues = chunks.map((text, index) => ({
      ...
      chunkIndex: index, classification: { type: "objection_knowledge_base", source: "agent-output" },
    }));
